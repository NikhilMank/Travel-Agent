...
State: **DynamoDB** for chats, messages, and LangGraph checkpoints.
...
| Endpoint | Purpose |
|----------|---------|
| `GET /api/chats` | List all chats, sorted by `updated_at` descending |
...
The chat endpoint accepts a `message` and `session_id`, and returns the response text, a completion flag, any tool calls made, worker names, and per-worker data sources.
...
backend/
  ...
  database/
    db.py                      — DynamoDB CRUD for chats, messages, and sync
  models/
...
frontend/
  src/
    App.jsx                    — Main React component (sidebar, messages, input, dark mode)
    App.css                    — All styles with CSS variables for theming
    api.js                     — API client functions (chats CRUD, sync, send message)
    main.jsx                   — React entry point
    index.css                  — Global reset styles
  .env                         — VITE_API_URL for local development
  index.html                   — HTML entry point
  package.json                 — Dependencies (react, react-markdown, vite)

lambda-package/                — Deployment package for Lambda (built from backend/)
  backend/                     — Source code
  deps/                        — Installed pip packages (boto3/botocore excluded)

travel-agent.zip               — Deployment ZIP for Lambda (< 50MB)
...
1. **Backend** — create a virtual environment, install dependencies, copy `.env.example` to `.env` and fill in your values, then start:
   ```bash
...
1. **Lambda** — upload `travel-agent.zip` as the function code. Set Python 3.12 runtime, handler as `backend.main.handler`. Configure environment variables matching your `.env`. Attach the `AWSLambdaBasicExecutionRole` and a custom policy with DynamoDB read/write and Bedrock `InvokeModel` permissions.

2. **API Gateway** — create a REST API, attach the Lambda as a proxy (`ANY /{proxy+}`), deploy to a stage. Enable CORS if needed.

3. **Frontend** — build the React app (`npm run build` in `frontend/`), upload `dist/` contents to an S3 bucket with static website hosting enabled.
...
- **Flat deployment ZIP over Lambda Layers** — simpler; single 50MB archive with all deps.
